...
#else
float AttenuationTerm(vec3 light_pos, vec3 pos, float range)
{
	vec3 v = light_pos - pos;
	float dist = length(v);
	v /= dist;

	float dist_inv_r = dist / range;
	dist_inv_r *= dist_inv_r;
	dist_inv_r *= dist_inv_r;

	float num = saturate(1 - dist_inv_r);
	num *= num;
	float ret = num / (dist * dist + 1);
	return ret;
}
#endif
